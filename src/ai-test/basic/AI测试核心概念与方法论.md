...
的方法论。
传统软件的行为由代码逻辑严格定义，给定输入A必然产生输出B。但AI模型的行为由训练数据和模型参数共同决定，同
一输入可能产生不同输出(尤其是LLM)。这意味着AI测试不能简单地assert expected == actual，而是需要：
1. 统计性评估
在大量样本上计算指标，而非单个用例通过/失败
2. 多维度评价
准确性、鲁棒性、公平性、安全性等多个维度
3. 人机结合
部分场景需要人工评估，无法完全自动化
4. 持续监控
模型部署后需要持续监控质量变化
...
最容易犯的错误：只看准确率！在类别不均衡时(如99%正常,1%异常)，一个总是预测'正常'的模型准确率高达99%
，但毫无用处。必须结合精确率、召回率和具体业务场景来选择指标。
### 4.3.2 回归模型指标
| 指标 | 含义 | 特点 |
|---|---|---|
...
当无法确定单个输入的正确输出时，可以利用输入之间的关系来验证。核心思想：如果输入做了某种变换，输出应该满足
某种关系。
```python
# 蜕变关系测试示例
# 关系1: 情感分析 - 否定应该翻转情感
