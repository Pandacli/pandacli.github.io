...
> 来源：[JVM 基础 - JVM 内存结构（Java 全栈知识体系）](https://pdai.tech/md/java/jvm/java-jvm-struct.html) · 作者：海星（公众号「JavaKeeper」），pdai 替换整理, 部分细节 本人自己优化

> 本文讲的是 **JVM 内存结构**，不要和 **Java 内存模型（JMM）** 混淆。
# 内存结构


> JVM 内存布局规定了 Java 程序运行过程中内存申请、分配、管理的策略。Java 虚拟机定义了若干程序运行期间会使用的运行时数据区,分成两大类：

- **线程私有**：程序计数器、虚拟机栈、本地方法栈[随着线程的生命周期开始和结束->创建和销毁]
- **线程共享**：堆、方法区 `method area`、堆外内存（Java 7 的永久代或 JDK 8 的元空间、代码缓存）



## 一、程序计数器 `Program Counter Register`

JVM 中的 PC 寄存器是对物理 PC 寄存器的一种抽象模拟，是一块较小的内存空间，可看作当前线程所执行字节码的**行号指示器**, 

### 1.1 作用

> PC 寄存器用来存储指向下一条指令的地址，即将要执行的指令代码，由执行引擎读取下一条指令。

- 使用 PC 寄存器存储字节码指令地址，是因为 CPU 需要`不停切换线程`，切换回来后要知道接着从哪开始执行。
- 多线程环境下，CPU 会不停做任务切换，为准确记录`各个线程正在执行的字节码指令地址`，PC 寄存器被设计为线程私有
- 它是一块很小的内存空间，`运行速度最快`的存储区域
...
- `字节码解释器`可以通过`改变计数器值`来选取下一条字节码指令
- **它是唯一一个在 JVM 规范中没有规定任何 `OutOfMemoryError` 情况的区域**

## 二、虚拟机栈 `Java Virtual Machine Stacks`

> 每个线程创建时都会创建一个虚拟机栈，内部保存一个个栈帧（Stack Frame），对应一次次 Java 方法调用，是线程私有的，生命周期和线程一致。

### 2.1 **特点**

- 栈是一种快速有效的分配存储方式，`访问速度 < 程序计数器`
- JVM 对虚拟机栈的操作只有两个：方法执行时`入栈`，方法结束`出栈`
- 栈不存在垃圾回收问题, 栈帧是线程私有的，生命周期与线程一致 
- 可以通过参数`-Xss`来设置线程的最大栈空间，栈的大小直接决定了函数调用的最大可达深度


### 2.2 栈的存储单位和内部结构

- 每个线程的栈中，数据以**栈帧Stack Frame**格式存在
- 正在执行的每个方法都对应一个栈帧
...
- 一个时间点只有一个活动栈帧`Activity Stack Frame`，即栈顶栈帧，称为**当前栈帧**，对应**当前方法**、**当前类**
- 执行引擎的所有字节码指令只针对当前栈帧操作

- 调用其他方法时，新栈帧`new Stakc Frame`被创建放在栈顶，成为新的当前栈帧
...
- 方法退出有两种方式：
 1. 正常 return 返回，会导致栈帧出栈
 2. 抛出异常  会导致栈帧出栈，并让异常处理机制处理


### 2.4 栈帧`Stack Frame` 的内部结构 

> 每次调用方法，JVM 就搭一个栈帧（工位）；方法一返回，工位就拆掉。所以"方法嵌套调用"就是工位一个压一个，栈大小 = 能摞多少层工位，工位越大（局部变量越多）摞的层数越少（递归容易栈溢出）。
...
#### 2.4.1 局部变量表 `Local Variables`

> 工位储物格。开工前就知道要挖几个格子，编译时就定死
...
- 局部变量只在当前方法调用中有效
- 参数值从 index0 开始存放 (工作带来的东西从 0 号格开始放)

**槽 Slot**：

**局部变量表是重要的 GC Roots，被其直接或间接引用的对象不会被回收** 

- 局部变量表基本存储单元是 Slot
- 32 位以内类型占用一个 Slot；64 位类型（long、double）占用两个连续 Slot (大件货占两格，小件占一格；byte/short/char/boolean 都先"加宽"成 int 再存)
...
#### 2.4.2 操作数栈 `Operand Stack`
> 工位上的"草稿纸"

- 后进先出
- 方法执行过程中，根据字节码指令入栈/出栈 ,字节码会 一步一步计算。
```
计算a+b 的过程 =  a  b  a+b  拆分成三步
把a 先入栈
把b 先入栈
把a 和 b 出栈，计算a+b 的结果，再入栈
```
- 用于保存计算过程的中间结果，是 JVM 执行引擎的工作区

- 方法刚开始时操作数栈为空

- 最大深度在编译期确定，保存在 Code 属性的 `max_stack` [最多能摞多高，编译时就定死了]
- 32 位类型占一个栈单位深度，64 位类型占两个
- 只能通过入栈和出栈访问，不能通过索引访问
- 方法有返回值时，返回值被压入当前栈帧的操作数栈，并更新 PC 寄存器[方法干完活，把结果往纸上一放，让调用方接着用]
- 数据类型必须与字节码指令严格匹配


**栈顶缓存（Top-of-stack-Cashing）**：
> HotSpot 的小聪明：最上面那张纸直接捏在手里（CPU 寄存器），不用每次都弯腰去桌上拿，快很多

HotSpot 将栈顶元素缓存在物理 CPU 寄存器中，降低对内存的读/写次数，提升执行效率。

#### 2.4.3 动态链接（指向运行时常量池的方法引用） 查"通讯录"

> 字节码里调用方法时，写的其实不是真实地址，而是一个名字（符号引用），像只写了"张三"没写他家门牌号。运行时得查通讯录（运行时常量池）才能找到真地址。

**JVM 方法调用相关概念**：

> 静态：编译器一看就知道是哪个方法（比如私有方法、静态方法），直接把门牌号写好，不用查
- **静态链接**：目标方法在编译期可知且运行期不变，将符号引用转为直接引用 
> 动态：多态调用——animal.eat()，到底执行猫的 eat 还是狗的 eat，运行期才知道，只能现场查
- **动态链接**：目标方法在运行期才能确定，转换过程具有动态性 


**虚方法和非虚方法**：
> 非虚方法：不用查通讯录的：静态、私有、final 方法、构造器、父类方法（super.xxx）


> 其他方法为虚方法：JVM 为多态准备的一张"方法名 → 真实地址"的速查表。类加载时建好，多态调用直接翻表，比一个个比对快
**虚方法表**：
...
> 用来存放调用该方法的 PC 寄存器的值 =工位上的"便签条"。
- 正常完成出口：执行引擎遇到方法返回字节码指令，如 ireturn、lreturn、freturn、dreturn、areturn、return ，即**干完活（遇到 ireturn/return），按便签条回调用处，工位拆除**
- 异常完成出口：方法内异常未被处理，通过异常表确定返回地址；不会给上层调用者产生返回值,直接拆工位把异常甩给上层
...
查看bytecode
``` java
 //方法被调用时，参数从局部变量表的 index 0 开始，按声明顺序连续存放。JVM 规范不关心你的 Java 代码长什么样，只约定"槽位编号 → 参数"的映射规则

 // 当前方法 开工前 已经决定了 a 和 b 要放到 0 和 1 号格，0 和1 号格被占用后，局部变量 c 要放到 2 号格
  @groovyx.ast.bytecode.Bytecode
  public static int add(int a,int b) {
    
    iconst_0 //把int const 0入栈，草稿纸上写个0
   
    istore 2 //弹出栈顶int，存入槽格 slot 2， 将纸上的0 存入到 2号槽格
   
    iload 0  //把 0号格 的 a 抄到纸上 

    iload 1 //把 1号格 的 b 抄到纸上 
   
    iadd  // 两张纸合并,纸上变成a+b 弹出栈顶两个int，相加，结果压栈
    
    istore 2 // 把结果 存入槽格 slot 2
   
    iload 2 // 再从2号格从结果拿出来放纸上 重新 压栈
   
    ireturn //返回栈顶int ,结束方法带着纸上这个数走人（回调用处），工位拆除
  }

```
![alt text](/assets/images/jvm/image.png)
